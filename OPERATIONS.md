...
`.github/workflows/sanity-backup.yml` exports the production dataset nightly and
keeps each export as a workflow artifact. Restore:

```sh
npx sanity dataset import <artifact>.tar.gz production --replace
```

from the repo root, logged in as an admin.

**Validating a backup without importing it** (the drill WCP ran 2026-08-31):
download the artifact, extract it, and check the pieces — every `data.ndjson`
line parses as JSON, the keystone documents are present, and the asset file
count matches `count(*[_type in ["sanity.imageAsset","sanity.fileAsset"]])`
live. That proves the artifact is a usable import file with zero risk.

**Do NOT rehearse the import into a scratch dataset without checking dataset
visibility first.** On free-tier Sanity projects, NEW datasets are created
PUBLIC ("Private datasets are not available") even when `production` itself is
private — so a rehearsal import can silently publish everything in the export.
Check what `sanity dataset create` says about visibility before any import
that is not into `production`. (Learned the safe way on WCP, whose export
holds the family directory.) Also: deploy/editor tokens cannot create datasets
(`sanity.project.datasets/create` grant) — dataset creation needs an admin
`sanity login`.
