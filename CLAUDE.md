...
- The generic "editable surface only" preview branch (hero + `flexibleSections` + closing CTA, with a note saying the code-owned middle renders on the live site) is now reached only by `404` and by any new singleton before it converts. Custom `page` docs preview in full (they are pure builder pages).
