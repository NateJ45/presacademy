...
- **Home previews in FULL fidelity**: its entire body is the shared `src/components/home/HomeBody.astro`, rendered by `index.astro` with build-time published data and by the preview route with draft data through the SAME query functions (each home query in `queries.ts` accepts an optional `fetcher`). One template, so the preview cannot drift from the page. **This is the pattern for converting the other singletons** when their approximation stops being good enough: extract the body into a shared component, parameterize its queries with a fetcher, branch in `src/pages/preview/[...slug].astro`.
- **Pages converted to the page builder also preview in FULL fidelity** (2026-08-26: privacy, accessibility, for-you, resources). Their bodies are Sanity section arrays rendered by the shared `src/components/SingletonPage.astro`, which the page and the preview route both use; the preview passes the draft fetcher into the same query function. Converting a page upgrades its preview automatically: add its type to `CONVERTED_PAGE_FETCHERS` in the preview route in the same commit.
- The REMAINING singleton pages still preview their **editable surface only**: hero, the `flexibleSections` builder blocks, and the closing CTA, with a note saying the code-owned middle renders on the live site. Custom `page` docs preview in full (they are pure builder pages).
- Path→type mapping lives in TWO places that must stay in sync: `SINGLETON_PREVIEW_PATHS` in `src/sanity/resolve.ts` and `SINGLETON_BY_PATH` in `src/pages/preview/[...slug].astro`.
...
Most fixed routes end with `<Sections sections={page?.flexibleSections} />`: the page-builder blocks an editor appends below the built-in content. Four routes have gone further (2026-08-26): `/resources`, `/privacy`, `/accessibility`, and `/for-you` render their whole body from that array through `src/components/SingletonPage.astro`, with `src/lib/default-sections.ts` supplying the same copy when the array is empty. The rest convert in later phases of `docs/superpowers/plans/2026-08-26-page-builder-conversion.md`.
