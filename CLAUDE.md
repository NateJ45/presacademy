...
- The OTHER singleton pages still preview their **editable surface only**: hero, the `flexibleSections` builder blocks, and the closing CTA, with a note saying the code-owned middle renders on the live site. Custom `page` docs preview in full (they are pure builder pages).
