...
- `node scripts/seed-academic-images.mjs` (add `--apply` to write) sets the home hero slideshow (`homePage.heroImages`, 6 images) and REPLACES every empty or church-era (`teach-*` / `study-*` / `community-*` / `place-*`) course cover + page hero with an academic CC0 placeholder from `src/assets/placeholders/acad-*.jpg`. Protects real editor images + already-seeded `acad-*` (idempotent); faculty headshots stay (pravatar). Supersedes the church-themed pools in `seed-placeholder-images.mjs` for the school look. (Curation tooling lives in gitignored `scripts/_*` scratch.)
- `node scripts/seed-editability.mjs` (add `--apply` to write) is the FULL editability seed (2026-06-15): render-neutral, only-empty + `createIfNotExists`, idempotent. It seeded the closing-CTA bands + the new fields + nav/footer menus across the 10 existing singletons, and CREATED the 5 previously-missing singletons (contact / privacy / accessibility / 404 / events, legal bodies as Portable Text) plus the `faqCategory` (5) / `faqItem` (11) / recurring `event` (3) collections, taking the dataset 39 -> 63 docs so Studio mirrors the live site. NOT seeded: `siteSettings.phone` + socials (need real values). Re-run safe.
...
| `/accessibility` | `src/pages/accessibility.astro` | Accessibility statement: `accessibilityPage` singleton + static fallback, twin of `/privacy`; barrier-report contact from `siteSettings` |
| `/[slug]` | `src/pages/[slug].astro` | Custom pages: the `page` collection + the 19-block page builder |
...
| CI/CD, staging preview, Sanity backups, uptime, hCaptcha spam protection (ops hardening) | `docs/agent/ci-cd-and-ops.md` |
