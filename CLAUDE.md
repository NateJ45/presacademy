...
17. **styled-components must land in ONE chunk, or the signed-in Studio dies** (bit 2026-08-26, in production, twice). styled-components is a peer dependency of both `sanity` and `@sanity/ui`, and only one copy exists on disk — but rolldown was splitting its code across two chunks, which means two module instances: the `ThemeProvider` mounted by one is invisible to the `useTheme()` call in the other, so every themed `@sanity/ui` component throws styled-components **error #18** as soon as the structure tool renders. The Studio shell and login screen are fine, so it only appears AFTER signing in, and the chunk named in the stack changes between builds (`layer-*`, `button-*`) which makes it look like a different bug each time. Fixed by the `advancedChunks` group in `astro.config.mjs`. Verify with `grep -l "errors.md#" dist/client/_astro/*.js` — it must list exactly ONE file.
18. **Studio plugins must match the `@sanity/ui` MAJOR that `sanity` itself uses** (bit 2026-08-26, in production). `sanity-plugin-media@5` was copied over from the WCP repo, but WCP runs sanity 6.4 (`@sanity/ui` v3) while this repo runs sanity 6.11 (`@sanity/ui` v4). npm nested a second, v3 copy of `@sanity/ui` for the plugin; v3 components read their theme through styled-components, found only the v4 theme context, and threw **styled-components error #18, "Accessing `useTheme` hook outside of a `<ThemeProvider>`"** the moment the structure tool rendered. The Studio shell and the LOGIN screen render fine, so the crash only appears after signing in. Fixed by moving to `sanity-plugin-media@^6` and `sanity-plugin-asset-source-unsplash@^7.0.25`. After adding or upgrading any Studio plugin, run `npm ls @sanity/ui --all` in BOTH packages and confirm a single major. (A v3 copy still hangs off `@sanity/astro`'s nested `@sanity/visual-editing`; that is fine because the Studio entry never imports visual-editing, so it lands only in the site's preview bundle.)
19. **Curling a page is not verifying it.** `/studio` returned 200 with real HTML while being completely broken at React mount. Anything that mounts a client framework has to be opened in a real browser with the console read. Same rule as the schema gotcha: a green build proves nothing about runtime.
20. **`src/components/ui/accordion.tsx` is customized** (removed the `h-(--radix-accordion-content-height)` lock, dropped `text-sm font-medium` from the trigger). Reinstalling via `npx shadcn add` reverts it; reapply the changes.
