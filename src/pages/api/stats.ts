...
// Squarespace and Wix both put a traffic panel in the editor, so staff look for
// one here. This is that panel's data, scoped honestly. Ported from the WCP
// site, 2026-08-28.
//
...
// that cannot be exercised locally: it needs the real token.)
//
// GATE
// The Studio preview cookie, the same unforgeable fingerprint the /preview
// routes trust (src/lib/preview-auth.ts). An editor gets it the first time they
// open Presentation; the tool's empty state says so. This is not dangerous
// data, but it is the school's data, and the cookie is free.
//
// CACHE
// A module-scope Map, ten minutes, keyed by today's UTC day. NO KV: this Worker
// has no KV binding, the account is close to the free tier's ~1k writes/day cap
// (the WCP cache is the account's only KV user), and this panel is read a
// handful of times a month. A cold isolate costs one Cloudflare round trip.
//
...
const SCRIPT_NAME = 'presacademy';
const GRAPHQL_URL = 'https://api.cloudflare.com/client/v4/graphql';

const QUERY_BY_DATE = `query PaSiteStatsByDate($accountTag: string!, $scriptName: string!, $since: Date!, $until: Date!) {
  viewer {
...
const QUERY_BY_HOUR = `query PaSiteStatsByHour($accountTag: string!, $scriptName: string!, $since: Time!, $until: Time!) {
  viewer {
    accounts(filter: { accountTag: $accountTag }) {
      workersInvocationsAdaptive(
        limit: 10000
...
export const GET: APIRoute = async (context) => {
  ...
  // ---- configuration --------------------------------------------------
  // Cast rather than trust generated types: CF_ANALYTICS_TOKEN is a Worker
  // SECRET (never in wrangler.jsonc, so `wrangler types` cannot know it) and
  // CF_ACCOUNT_ID is a plain var. Same shape as src/lib/preview-auth.ts.
  const runtime = env as { CF_ANALYTICS_TOKEN?: string; CF_ACCOUNT_ID?: string };
  const token = runtime.CF_ANALYTICS_TOKEN;
  const accountTag = runtime.CF_ACCOUNT_ID;
