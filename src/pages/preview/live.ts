...
// ~nothing, and an edit costs one page refetch. Never reintroduce an interval
// poll here.
//
// The GROQ filter: signal when THIS page's doc changes (draft or published
// id), or when any shared / non-`page` doc changes (courses, faculty, terms,
// settings... all can appear on any page via the auto-updating sections) — but
...
import { projectId, dataset, apiVersion } from '@/lib/cms-preview';
...
export const GET: APIRoute = async ({ cookies, url }) => {
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      // instead of a reconnect storm. The manual ⟳ still works during the gap,
      // and the overlay does a catch-up refetch on reconnect, so no edit is
      // lost. (The connection also only stays open while the preview tab is
      // visible; the overlay closes it when the tab is hidden.)
      send('retry: 15000\n\n');
