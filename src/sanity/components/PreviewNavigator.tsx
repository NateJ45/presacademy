...
//  - Grouping: "Main pages" (the built-in singletons, in site-nav order) vs
...
async function fetchRows(client: ReturnType<typeof useClient>): Promise<NavRow[]> {
  ...
  const [singletons, pages] = await Promise.all([
    ...
    client.fetch<{ _id: string; _type: string; title?: string; slug?: { current?: string } }[]>(
      '*[_type == "page" && archived != true]{ _id, _type, title, slug }',
    ),
  ]);
...
export function PreviewNavigator() {
  ...
  useEffect(() => {
    refetch();
    // Live refresh: any page mutation (rename, publish, new page) → refetch
    // after a short settle. visibility:'query' waits until it is queryable.
