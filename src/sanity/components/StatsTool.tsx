...
// =============================================================================
// Registered as a navbar tool in sanity.config.ts. Read-only. It calls the
// site's own /api/stats endpoint, which reads Cloudflare's Workers analytics
// for this Worker and returns 28 days of daily counts. Ported from the WCP
// site, 2026-08-28.
//
...
// "visitors": it would be wrong, and somebody would quote it.
//
...
function ProblemCard(props: { problem: Problem; onRetry: () => void }) {
  ...
  const body =
    problem.reason === 'no-session' ? (
      <Stack space={4}>
        <Text size={1}>
          The stats panel needs to know that you work here. It gets that from the site preview.
        </Text>
        <Stack space={2} as="ol">
