...
Lenis init is wrapped in `requestIdleCallback` so it doesn't compete with first-paint work, and since 2026-09-04 it only runs on wheel devices: `(pointer: fine)` AND `(min-width: 1024px)`. Phones and tablets keep native inertial scrolling and never download the Lenis chunk (it was ~760ms of throttled main-thread time on the mobile Lighthouse run for an effect touch never shows). `window.lenis` is therefore undefined on touch devices; every caller already falls back to native `scrollTo`.
...
**Home split-hero — `HeroSlideshow.astro` (CSS-only, no JS).** The home page (`index.astro`) renders its framed hero image (the `aspect-[3/2] lg:aspect-[4/5]` box beside the headline) with `HeroSlideshow`, fed `homePage.heroImages`. It is pure CSS: one `@keyframes` whose fade/zoom stops and each slide's `animation-delay` are GENERATED from the image count, so the cross-fade hands off cleanly for any number of slides with no interval timer. Each slide zooms `scale(1.04)` -> `scale(1.15)` from a rotating `transform-origin` (the pan). The keyframes are injected per-instance via `<style is:inline set:html>` under a `hkb-<id>` class so multiple instances never collide. First slide eager/high-priority (LCP), the rest DEFERRED + decorative (`alt=""`): they render through `SanityImage defer`, which puts the real URLs in `data-src`/`data-srcset`/`data-sizes` behind a blank 1x1 GIF, and a small bundled `<script>` in HeroSlideshow swaps them in ~800ms after the `load` event (and again on `astro:page-load`). `loading="lazy"` was not enough because every slide sits stacked inside the hero box, so all of them downloaded next to the LCP image (three ~80KB fetches at high priority on the 2026-09-04 mobile Lighthouse run). `0` images -> the empty well, `1` -> static, `2+` -> slideshow. **It requests 4:5-CROPPED variants from Sanity** (`height = width * 1.25`): the desktop crop is a tall portrait, so a width-only request serves a short landscape that object-cover then upscales (wasting the source pixels). Because of that, **hero source images must be HIGH-RES** (~2000px+); the 4:5 crop magnifies any shortfall, so ~960px images visibly pixelate. (Built 2026-06-14; the home split-hero previously rendered only `heroImages[0]`. The 4:5 crop-serve + high-res sourcing landed in the same-day pixelation fix.)
