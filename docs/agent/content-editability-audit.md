...
**Highest-leverage editor win (works on static):** a per-document root-input **banner** at the top of each page form — "This is your _Home_ page" + a **"View this page on the live site ↗"** deep link (from the existing `urlForDoc()`) + the publish→rebuild reminder. Reuses the `CharacterCountInput` root-input pattern. Directly maps Studio → live page.
