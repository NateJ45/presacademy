...
1. **Install** (`npm ci`).
2. **Generate Sanity types** (three attempts: `sanity schema extract` intermittently core-dumps on the hosted runners), then **fail if `src/lib/sanity.types.ts` is stale** (`git diff --quiet`). The build does not run typegen, so the committed types must be regenerated by hand; this step catches the exact bug that shipped on 2026-06-14.
3. **Type check** (`npx astro check`).
4. **Lint** (`npm run lint`, eslint).
5. **Format check** (`npm run format:check`, prettier with the astro + tailwind plugins).
6. **Unit tests** (`npm run test:unit`, `node --test`).
7. **Build** with `PUBLIC_SANITY_PROJECT_ID=""` so `sanityFetch` returns empty-state fallbacks (no credentials needed; the build is designed to succeed without a Sanity project). The Studio is bundled into `/studio` by this same build.
8. **Internal link check** (`npm run check:links`, linkinator over `dist/client`; `/studio`, `/preview` and `/api` are skipped).
