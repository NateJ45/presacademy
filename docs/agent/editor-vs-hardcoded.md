...
> **School-build status (2026-06-14): the gap is largely closed.** The lay-school rebuild had originally shipped many sections as hardcoded literals on top of reused church schemas, so the blanket "all content is editor-driven" claim below was NOT true for a while. The Phases 0-4 editability pass fixed the worst of it: home, about, the detail-page silent-data-loss fields (course syllabus / seats / price unit; faculty specializations / years / email; event all-day), Get Started's "Request information" panel, the faculty empty-state, and the footer **funder** line are now editor-driven, and seed-page-copy.mjs filled the new fields so Studio mirrors the live site. A short list of **structural scaffolding** is intentionally still hardcoded (see below). The live page-by-page map is **[`content-editability-audit.md`](content-editability-audit.md)** — read it for the per-page detail; the section examples below still lean on the finished *church* build's page set. **Update 2026-06-15:** the gap is now FULLY closed. `scripts/seed-editability.mjs` seeded the closing-CTA bands on every singleton and created the previously-missing contact/privacy/accessibility/404/events singletons plus the faqItem/faqCategory/event collections (39 -> 63 docs). The page-builder block list further down is stale (the arch was retired; the current 19 blocks, including Resources/downloads, Key dates, Tuition tiers, and Testimonials, are in `page-architecture.md`).

## Sanity is the single source of truth

**Nearly all content on the site is editor-driven through Sanity, and the populated fields are seeded to mirror the live site.** Page copy, headings, body text, buttons and links, images, the navigation menus, SEO titles/descriptions, the worship service time, and contact details live in Sanity. To change anything a visitor reads, sees, or clicks, edit it in Sanity Studio; the site rebuilds and the change appears a few minutes later. The exceptions are a deliberate set of **structural scaffolding** (detail-page section labels like "At a glance" / "Degrees" / "Who this is for", the catalog/faculty filter legends in the React islands) and **brand constants** (the colophon "PA" monogram and the "PC(USA)" tag); fielding those would bloat Studio with dozens of never-touched fields. For the exact school-page state, see [`content-editability-audit.md`](content-editability-audit.md).
