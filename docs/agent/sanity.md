...
- `studio/resolve.ts` maps documents to `/preview/*` routes and back (`SINGLETON_PREVIEW_PATHS` is the singleton table; keep it in sync with `SINGLETON_BY_PATH` in `src/pages/preview/[...slug].astro`).
- `studio/components/PreviewNavigator.tsx` docks a page list beside the preview: main pages vs custom pages, status dots (amber = published with unpublished edits, hollow = never published), a live-page link per published row, and "+ New page".
